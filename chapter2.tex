...
Another discarded attribute is the linear mass balance based on the catchment areas of the glacier. This is similar to the linear mass balance which was finally used to train the algorithms. The point-wise mass balance based on the catchment area however is calculated only from the sum of the mass balances of the grid cells, belonging to branches of the glacier which actually flow into the considered point. For glaciers with only one branch the two mass balance would coincide. To better explain this discarded feature one can look at the right image in Fig. \ref{fig:mb_map} which shows this type of mass balance. The top right part of the image shows a very dark blue branch of the glacier corresponding to very low values of mass balance. The same branch with the mass balance finally used for training (left image), shows instead a very high mass balance value. The mass balance computed on this branch on the right image in fact, doesn't take into account all the other branches of the glacier, as obviously the ice can not flow from these other branches into this one. In the linear mass balance used to train the models this is not taken into account, leading to a much higher mass balance for this area of the glacier. The reason this feature was discarded is that, as clearly visible in the image on the right, this was leading to zones with very discontinuous mass balance values. When making predictions with the machine learning models this feature was strongly influencing the outcome of the prediction, resulting in a very discontinuous ice thickness distribution.
...
In order to avoid these problems we decided to arbitrarily train all the models on region 11 of the RGI inventory, corresponding to Central Europe and in particular the Alps. This region accounts for a total of 3297 glaciers. Of those glaciers however only 106 have thickness observations in the GlaThiDa. After assigning each of these observations to a grid cell in each glacier map, the total number of thickness measurements available to train the models are 8800. When cross validating the model as described in section \ref{training}, a total of 6600 measurements are then used to train the models and 2200 to test their performances.
