...
\begin{figure}[!tp]
	...
	\caption{Representation of shuffling the data for a sample data-set. Input values for one features are shuffled randomly switching the data with each other. The value to predict however is not switched leading to an input value for the permuted feature which is different from the ``true'' one.}
